...
> **Goal:** Make `party-cli`'s worker sidebar, master tracker, and adjacent party picker feel like `scry` siblings by porting scry's pane chrome, theme tokens, typography hierarchy, height-aware footer/status handling, and terminal-theme-safe ANSI color usage into the existing `party-cli` render surfaces.
>
> **Architecture:** Introduce a local bordered-pane helper layer in `tools/party-cli/internal/tui/` that owns both width and height budgeting, plus a shared token vocabulary that reaches the raw ANSI formatting seam in `tools/party-cli/internal/picker/`. Steady-state TUI views render as bordered panes with embedded footers; a full-width status bar is reserved for transient errors or active input modes only when height allows. The worker and tracker remain the same behavioral surfaces; only the render contract changes. The picker remains raw ANSI text for `fzf`, but its colors align to the same scry token system.
>
...
This plan covers `tools/party-cli/internal/tui/` and the picker formatting seam in `tools/party-cli/internal/picker/`: shared styles, pane chrome, worker sidebar, tracker list, manifest viewer, tracker composer, height-aware footer/status behavior, picker list/preview ANSI formatting, and corresponding tests. It does not change `party-cli` session/message/tmux behavior, picker ordering, or introduce a cross-repo dependency on `scry`.
...
- [ ] [Task 2](./tasks/TASK2-restyle-worker-sidebar-shell.md) — Apply the new chrome and explicit label/value/help hierarchy to the worker sidebar and error shell, including short-pane footer fallback and tall-pane transient status behavior (deps: Task 1)
...
| Worker sidebar label/value/help hierarchy | Task 1 | Worker shell, evidence block, tests | Task 2 | `sidebarLabelStyle = StatusFg`; metadata values default to `Muted`; help/footer text stays `Muted` + `Faint` |
...
**Validation:** Every row above is intentionally shared or fan-out. Task 1 creates the primitives and vocabulary; Task 2 and Task 3 consume them inside Bubble Tea surfaces; Task 4 applies the same token contract at the raw ANSI picker seam so the family resemblance does not stop at the `fzf` boundary.
...
| After Task | State |
|------------|-------|
| Task 1 | `party-cli` has scry-aligned token names, bordered pane helpers, a `compactHeightThreshold` policy, sidebar label/value/help styles, footer-vs-status-bar rules, and helper tests for ANSI-aware title rendering |
| Task 2 | Worker mode and the error shell render with bordered chrome, bright labels, muted/semantic values, steady-state footer hints, and short-pane fallback rules |
...
- The worker sidebar hierarchy problem is real in the present code: labels and values are currently rendered with little separation, with most non-semantic lines falling back to `dimStyle` in `tools/party-cli/internal/tui/sidebar.go:19-96` and `tools/party-cli/internal/tui/style.go:15-26`.
...
- [ ] Worker sidebar labels render brighter than metadata values and help text
...
- **Codex peek window** (`tools/party-cli/internal/tui/sidebar_popup.go`): Currently uses `tmux display-popup` with `less -R` and has no custom styling. A future pass could pipe the captured pane content through a Go formatter that adds scry-style bordered chrome to the popup. Low priority — the tmux popup frame already provides minimal visual structure.
