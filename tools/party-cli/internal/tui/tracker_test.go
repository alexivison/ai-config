...
func TestTrackerViewShowsHierarchy(t *testing.T) {
	...
	if !strings.Contains(view, "M party-1230") {
		t.Fatalf("expected M icon on master metadata row, got:\n%s", view)
	}
	if !strings.Contains(view, "W party-1231") {
		t.Fatalf("expected W icon on worker metadata row, got:\n%s", view)
	}
	if !strings.Contains(view, "S party-1236") {
		t.Fatalf("expected S icon on standalone metadata row, got:\n%s", view)
	}
...
func TestTrackerViewShowsPartyTitleInHeader(t *testing.T) {
	...
	expectedTitle := renderTrackerANSI(paneTitleStyle, "M Project Alpha (party-master)")
	if !strings.Contains(view, expectedTitle) {
		t.Fatalf("expected titled tracker header with master role letter, got:\n%s", view)
	}
}
...
func TestTrackerViewUsesWorkerChromeForWorkerHeader(t *testing.T) {
	...
	expectedTitle := renderTrackerANSI(paneTitleStyle, "W Investigate (party-worker)")
	if !strings.Contains(view, expectedTitle) {
		t.Fatalf("expected worker tracker header with worker role letter, got:\n%s", view)
	}

	// Pane title must no longer carry any agent-color foreground.
	codexTinted := renderTrackerANSI(paneTitleStyle.Foreground(agentIdentityStyle("codex").GetForeground()), "W Investigate (party-worker)")
...
func TestTrackerViewFallsBackToSessionHeaderWhenTitleMissing(t *testing.T) {
	...
	expectedTitle := renderTrackerANSI(paneTitleStyle, "S party-current")
	if !strings.Contains(view, expectedTitle) {
		t.Fatalf("expected session ID fallback header with standalone role letter, got:\n%s", view)
	}
...
// new role-letter + default-foreground rendering. Each session type
// (master / worker / standalone) gets its letter prefix; agent color
// must not leak in.
func TestTrackerPaneTitleUsesRoleGlyph(t *testing.T) {
	cases := []struct {
		...
		{sessionType: "master", wantGlyph: "M"},
		{sessionType: "worker", wantGlyph: "W"},
		{sessionType: "standalone", wantGlyph: "S"},
	}
...
func TestTrackerRenderSessionRowSelectedRowTintCoversStyledLines(t *testing.T) {
	...
	selectedMeta := renderTrackerANSI(selectedRowStyle.Inherit(metaTextStyle), "W "+row.ID)
