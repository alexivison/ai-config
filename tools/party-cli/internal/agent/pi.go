...
// agent-local skill root while sharing the generic role-aware contract.
...
(2) Spawn workers with party-cli spawn [title] or /party-dispatch. Workers have no companion by default; pass --companion <agent> to attach one. If --primary X --companion X (or any path where the resolved primary equals the resolved companion) the spawn errors before any tmux work — pick a different companion. Relay observations (file:line, log excerpts, scope, acceptance) via party-cli relay <worker-id> "message" — let workers pick the fix; prescribe only when asked, mechanical, or stuck 2+ attempts. Broadcast to all workers with party-cli broadcast "message", inspect workers with party-cli workers or party-cli read <worker-id>, and require workers to report back via party-cli report from the worker session. Master sessions have no companion pane (the sidebar shows the tracker); all delegation goes through workers, so do not attempt tmux-companion dispatch from the master.
...
(2) Use your own/current agent role-aware transport skill only; dispatch the companion via ~/.pi/agent/skills/agent-transport/scripts/tmux-companion.sh when you need review, planning, or parallel investigation.
